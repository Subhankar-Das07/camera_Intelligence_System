document.addEventListener("DOMContentLoaded", () => {
    ...
    function handleFileUpload(file) {
        ...
        fetch("/api/upload", { method: "POST", body: formData })
            ...
            .then(data => {
                ...
                // Notify guardian module of new source (guardian.js defines this; harmless if absent)
    ...
    roiCanvas.addEventListener("click", (e) => {
        if (pipelineSelect.value === "room_guardian") return; // guardian.js handles its own canvas events
    ...
    function checkRunReady() {
        // room_guardian has its own Start Guarding button — run-btn is not used
        if (pipelineSelect.value === "room_guardian") { runBtn.disabled = true; return; }
    ...
    pipelineSelect.addEventListener("change", (e) => {
        ...
        const isGuardian      = e.target.value === "room_guardian";
        ...
        // Hide standard controls when guardian pipeline is active (guardian.js owns its UI)
        if (machineSettings) machineSettings.style.display = (isFallDetection || isGuardian) ? "none" : "block";
        if (roiControls) {
            roiControls.style.display = (isFallDetection || isGuardian) ? "none" : "block";
            // NOTE: for room_guardian, guardian.js manages the canvas visibility itself.
            // Only hide the canvas for fall_detection. Clearing the inline style for guardian
            // lets guardian.js take over without fighting an overriding inline style.
            if (isGuardian) {
                roiCanvas.style.display = "";   // clear inline style — guardian.js takes over
            } else {
                roiCanvas.style.display = isFallDetection ? "none" : "block";
            }
        }
        if (controlActions) controlActions.style.display = isGuardian ? "none" : "";
